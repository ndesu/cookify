...
export function Inventory({ queryURL }) {
    ...
    const removeIngredient = (ingredientToRemove) => {
        setIngredientList(ingredientList.filter(item => item !== ingredientToRemove));
    }
    ...
    return (
        <div className={styles.inventoryContainer}>
            ...
            <div className={styles.title}>
                <h1 className={styles.h1}>ingredients in your <em>kitchen</em></h1>
            </div>
            <div className={styles.top}>
                ...
                <button className={styles.aButton} onClick={addIngredient}><em>submit</em></button>
            </div>
            <div className={styles.gridContainer}>
                <ul className={styles.ingredientGrid}>
                    {ingredientList.map((item, i) => 
                        <li key={i} onClick={() => removeIngredient(item)}>{item.label}</li>
                    )}
                </ul>
                <button className={styles.aButton} onClick={findRecipes}>find recipes!</button>
            </div>
        </div>
    );
}
